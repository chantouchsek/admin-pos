      ...
      <b-card header-tag="header" class="card-accent-success">
        <div slot="header">
          ...
          <div class="card-header-actions">
            ...
            <b-link href="#" class="card-header-action btn-close" :to="{ name: 'customer.create' }"
                    v-b-tooltip.hover
                    title="New Customer"
            >
              <i class="icon-plus"></i>
        ...
        <b-row>
          <b-col md="6" class="my-1">
            <b-pagination
              :total-rows="customer.pagination.totalCount"
              :per-page="limit"
              v-model="currentPage"
              class="my-0"
  ...
  export default {
    ...
    methods: {
      ...
      setQuery: debounce(async function (query) {
        const vm = this
        await vm.$store.dispatch('customer/all', (proxy) => {
          proxy.setParameters({
            'q': query,
       ...
       */
      destroy (customer) {
        this.$store.dispatch('customer/destroy', customer)
      }
    },
    /**
