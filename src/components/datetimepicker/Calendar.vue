  ...
  <div :class="`${pre}`">
    ...
    <div :class="`${pre}-body`">
      <div :class="`${pre}-days`">
        ...
        <a v-for="(j,i) in days" @click="is($event)&&(day=j.i,ok(j))"
           :class="[(j.p||j.n)?`${pre}-date-out`:'',status(j.y,j.m,j.i,hour,minute,second,'YYYYMMDD')]"
           :key="i">{{j.i}}</a>
      </div>
      <div :class="`${pre}-months`" v-show="showMonths">
        <a v-for="(i,j) in local.months" @click="is($event)&&(showMonths=(m==='M'),month=j,(m==='M'&&ok('m')))"
           :class="[status(year,j,day,hour,minute,second,'YYYYMM')]" :key="j">{{i}}</a>
      </div>
      <div :class="`${pre}-years`" v-show="showYears">
        <a v-for="(i,j) in years" @click="is($event)&&(showYears=(m==='Y'),year=i,(m==='Y'&&ok('y')))"
      ...
      <div :class="`${pre}-hours`" v-show="showHours">
        ...
        <a v-for="(j,i) in 24" @click="is($event)&&(showHours=false,hour=i,ok('h'))"
           :class="[status(year,month,day,i,minute,second,'YYYYMMDDHH')]" :key="i">{{i}}</a>
      </div>
      <div :class="`${pre}-minutes`" v-show="showMinutes">
        ...
        <a v-for="(j,i) in 60" @click="is($event)&&(showMinutes=false,minute=i,ok('h'))"
           :class="[status(year,month,day,hour,i,second,'YYYYMMDDHHmm')]" :key="i">{{i}}</a>
      </div>
      <div :class="`${pre}-seconds`" v-show="showSeconds">
        ...
        <a v-for="(j,i) in 60" @click="is($event)&&(showSeconds=false,second=i,ok('h'))"
           :class="[status(year,month,day,hour,minute,i,'YYYYMMDDHHmmss')]" :key="i">{{i}}</a>
      </div>
    </div>
    <div :class="`${pre}-foot`" v-if="m==='H'">
      <div :class="`${pre}-hour`">
        ...
        <span>:</span>
        <a :title="local.minuteTip" @click="showMinutes=!showMinutes,showHours=showSeconds=false"
           :class="{on:showMinutes}">{{minute|dd}}</a>
        <span>:</span>
        <a :title="local.secondTip" @click="showSeconds=!showSeconds,showHours=showMinutes=false"
