  ...
  <div class="cart">
    ...
    <div class="cart-items" v-show="showCart">
      <table class="cartTable">
        <tbody>
        <tr class="product" v-for="product,index in cart" :key="`product-${index}`">
          <td class="align-left">
            <div class="cartImage" @click="removeProduct(product)"
                 v-bind:style="{ backgroundImage: url( product.image ) }"
                 style="background-size: cover; background-position: center;"><i class="close fa fa-times"></i>
            </div>
          </td>
          <td class="align-center">
            <button @click="quantityChange(product, 'decriment')"><i class="close fa fa-minus"></i></button>
          </td>
          <td class="align-center">{{ cart[$index].quantity }}</td>
          <td class="align-center">
            <button @click="quantityChange(product, 'incriment')"><i class="close fa fa-plus"></i></button>
          </td>
...
<script>
  export default {
    name: "cart", props: ['checkoutBool', 'cart', 'cartSize', 'cartSubTotal', 'tax', 'cartTotal'],
