<template>
  <div class="product-list">
    <h1>Products</h1>
    <div class='products'>
      <div v-for='(product,index) in productsData' :track-by='index' class='product' :key="`product-${index}`">
        <div class='image' @click='viewProduct(product)' :style="{ 'background-image': 'url(' + product.image + ')' }"
    ...
    <div class='modalWrapper' v-show='showModal'>
      ...
      <div class='modal'>
        ...
        <div class='imageWrapper'>
          <div class='image' :style="{ 'background-image': 'url(' + modalData.image + ')' }"
               style='background-size: cover; background-position: center;' @mouseover='imageMouseOver($event)'
               @mousemove='imageMouseMove($event)' @mouseout='imageMouseOut($event)'></div>
        </div>
        <div class='additionalImages' v-if='modalData.images'>
          <div v-for='(image,index) in modalData.images' class='additionalImage' :key="`image-${index}`"
               ...
               style='background-size: cover; background-position: center;' @click='changeImage(image.image)'
          ></div>
        </div>
        ...
        />
        <button class="btn btn-success" @click='modalAddToCart(modalData)'>Add to Cart</button>
      </div>
    </div>
  </div>
</template>
